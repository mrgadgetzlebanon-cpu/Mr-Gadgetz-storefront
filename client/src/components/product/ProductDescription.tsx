...
function parseDescriptionWithHiddenTag(html: string): { description: string; specs: string } | null {
  const hiddenTagRegex = /<span\s+id\s*=\s*["']?hidden["']?\s*(?:\/>|>\s*<\/span>)|<\/hidden>|<!--\s*hidden\s*-->|<div\s+id\s*=\s*["']?hidden["']?\s*(?:\/>|>\s*<\/div>)/i;
...
export function ProductDescription({ descriptionHtml, description, specs }: ProductDescriptionProps) {
  ...
  const splitContent = descriptionHtml ? parseDescriptionWithHiddenTag(descriptionHtml) : null;
  ...
  const showBothColumns = hasDescription && hasSpecs;

  return (
    <div className={showBothColumns ? "grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12" : "w-full"}>
      {hasDescription && (
        <div className="space-y-4">
          <h3 className="font-display font-bold text-xl border-b border-border/50 pb-3">
            Description
